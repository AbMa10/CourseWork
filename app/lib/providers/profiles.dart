...
class Profiles with ChangeNotifier {
  ...
  Future<Profile> findById(String id) async {
    ...
    try {
      ...
      Profile curProfile = Profile(
          ...
          complains: prodData['complains'],
          wasDeleted: prodData['wasDeleted']);
  ...
  Future<void> updateProfile(String id, Profile newProfile, {bool flag = false}) async {
    final url =
        'https://flutter-shop-6df73-default-rtdb.firebaseio.com/profiles/$id.json?auth=$authToken';
      ...
      body: json.encode(
        {
          ...
          'complains': newProfile.complains,
          'wasDeleted': newProfile.wasDeleted
        },
      ),
    );
    if (!flag && curProfile.id == newProfile.id) {
      curProfile = newProfile;
    }
    notifyListeners();
  }
