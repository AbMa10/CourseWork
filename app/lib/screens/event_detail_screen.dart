...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  void checkForComplain() async {
    currentProfile = await Provider.of<Profiles>(
      context,
      listen: false,
    ).findById(Profiles.curProfileId);
    if (currentProfile.id == loadedEvent.profileId) {
      setState(() {
        needToComplain = false;
        checkForNeedToComplain = true;
      });
    }
  }
  ...
  void sendComplain() async {
    ...
    //   context,
    //   listen: false,
    // ).findById(loadedEvent.profileId);
    ...
    print("LLLLLLLLLL ${currentProfile}");
    // print("LLLLLLLLLL ${Profiles.curEmail}");
    await showDialog<Null>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Вы уверены, что хотите пожаловаться на мероприятие?',
          style: TextStyle(
            fontSize: 25,
        ...
        actions: <Widget>[
          ...
          TextButton(
            ...
            onPressed: () async {
              if (currentProfile.complains[loadedEvent.id] == true) {
                Navigator.of(ctx).pop();
                return;
              } else {
                print("TESTTTT ${loadedEvent.countOfComplains}");
                loadedEvent.countOfComplains++;
                print("TESTTTT ${loadedEvent.countOfComplains}");
                currentProfile.complains[loadedEvent.id] = true;
                Navigator.of(ctx).pop();
                await Provider.of<Events>(context, listen: false)
                    .updateEvent(loadedEvent.id, loadedEvent);
  ...
  bool checkForNeedToComplain = false;
  @override
